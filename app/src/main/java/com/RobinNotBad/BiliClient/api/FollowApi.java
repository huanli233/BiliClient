...
public class FollowApi {
    ...
    public static int searchFollowingList(long mid, String keyword, List<UserInfo> userList) throws IOException, JSONException {
        String url = "https://api.bilibili.com/x/relation/followings/search?vmid=" + mid + "&name=" + URLEncoder.encode(keyword, "UTF-8");
        JSONObject callback = NetWorkUtil.getJson(url);
        if (callback.optInt("code", -1) != 0)
            throw new JSONException(callback.optInt("code", -1) + "：" + callback.optString("message", "未知API错误"));
        JSONObject data = callback.getJSONObject("data");
